...
export default function Home() {
  ...
  let [delimiter, setDelimiter] = useState("character");
      ...
      <div className="flex grow divide-x divide-gray-700">
        <div className="flex w-1/2 flex-col gap-4 pt-4">
          ...
          <div className="h-0 grow overflow-y-scroll px-4">
            <p className="whitespace-pre-wrap text-sm text-gray-400 md:text-base">
              {text}
            </p>
          </div>
        </div>
...
const greatGatsby = `In my younger and more vulnerable years my father gave me some advice that I’ve been turning over in my mind ever since.
...
function getNextChars(n: number) {
  const result = greatGatsby.slice(position, position + n);
  position += n;
  return result;
}
